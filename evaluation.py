...
def exec_predict_and_test(envs, test_file, forecast_module, flag='predict'):
    ...
    if 'predict' == flag:
        envs["path_to_test_x"] = os.path.join(envs["path_to_test_x"], test_file)

        return {
            "prediction": forecast_module.forecast(envs)
        }
    elif flag == 'test':
        envs["path_to_test_y"] = os.path.join(envs["path_to_test_y"], test_file)
...
def predict_and_test(envs, path_to_data, forecast_module, idx, flag='predict'):
    ...
    # add by xiaotian
    return exec_predict_and_test(envs, idx, forecast_module, flag)
...
def evaluate(path_to_src_dir):
    ...
    # add by xiaotian
    path_x_original = envs['path_to_test_x']
    path_y_original = envs['path_to_test_y']
    items_x = os.listdir(envs['path_to_test_x'])
    items_y = os.listdir(envs['path_to_test_y'])
    files_x = sorted(items_x)
    files_y = sorted(items_y)

    assert len(files_x) == len(files_y)
    num_files = len(files_x)
    NUM_MAX_RUNS = num_files
    print("number of prediction to test: ", NUM_MAX_RUNS)

    for i in range(NUM_MAX_RUNS):
        pred_res = predict_and_test(envs, PRED_DIR, forecast_module, files_x[i], flag='predict')
        ...
        gt_res = predict_and_test(envs, TAR_DIR, forecast_module, files_y[i], flag='test')
        ...
        # add by xiaotian: reset original path
        envs['path_to_test_x'] = path_x_original
        envs['path_to_test_y'] = path_y_original
...
def zip_compress(to_zip, save_zip_name):
    ...
    if not os.path.exists(save_zip_dir):
        ...
        print('create new folder %s'% save_zip_dir)
    ...
    if not os.path.isdir(os.path.abspath(to_zip)):
        if os.path.exists(os.path.abspath(to_zip)):
            f.write(to_zip)
            f.close()
            print('%s is compressed as %s' % (to_zip, save_zip_name))
        else:
            print('%s target file does not exist' % os.path.abspath(to_zip))
    else:
        if os.path.exists(os.path.abspath(to_zip)):
            zipList = []
            # read all files
            ...
            # compress all files
            for i in zipList:
                f.write(i, i.replace(to_zip, '')) # not to include the path to_zip
            f.close()
            print('%s is compressed as %s' % (to_zip, save_zip_name))
        else:
            print('%s target file does not exist' % os.path.abspath(to_zip))
...
if __name__ == "__main__":
    ...
    import os
    test_name = 'test-1'
    output_name = test_name + '.zip'
    zip_dir = '/var/paddle/sub/tests/' + test_name
    target_file = '/var/paddle/sub/tests/' + output_name
    zip_compress(zip_dir, target_file)
    print("target test file:", target_file)

    # import paddle.distributed as dist

    # dist.spawn(eval, args=(target_file))
    eval(target_file)
