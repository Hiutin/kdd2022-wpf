...
from metrics import regressor_scores, regressor_detailed_scores, regressor_detailed_scores_train
...
def train_and_evaluate(config, train_data, valid_data):
    ...
    save_path = os.path.join(config['output_path'], "seed%d" % config['seed_num'])
    ...
    for epoch in range(config['epoch']):
        ...
        if paddle.distributed.get_rank() == 0:
            ...
            if best_score == valid_r['score']:
                patient = 0
                save_model(save_path, model, steps=epoch, valid_info=valid_r, seed=config['seed_num'])
            else:
                patient += 1
...
def visualize_prediction(input_batch, pred_batch, gold_batch, tag, epoch_num, path_save='../visualization/'):
    plt.figure()
    for i in range(1, 5):
        ...
        ax.plot(
            ...
            label="pred")
        ax.legend()
    plt.savefig(path_save + tag + str(epoch_num) + "_vis.png")
    plt.close()
...
def evaluate(valid_data_loader,
    ...
    visualize_prediction(
        ...
        np.sum(gold_batch[:, :, :, 0], 0) / 1000., tag, epoch_num)
