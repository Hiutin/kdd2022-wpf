...
def regressor_detailed_scores(predictions, gts, raw_df_lst, settings):
    ...
    for i in range(capacity):
        ...
        gt = gts[i]
        raw_df = raw_df_lst[i]
        _mae, _rmse = turbine_scores(prediction, gt, raw_df, output_len, idx=identifier)
        ...
        latest_mae, latest_rmse = turbine_scores(prediction, gt, raw_df, day_len, idx=identifier)
...
def regressor_detailed_scores_train(predictions, gts, raw_df_lst, capacity,
                              output_len):
    """
    Desc:
        Some common metrics for regression problems
    ...
    for i in range(capacity):
        ...
        gt = gts[i]
        raw_df = raw_df_lst[i]
        _mae, _rmse = turbine_scores_train(prediction, gt, raw_df, output_len, 1)
        all_mae.append(_mae)
        all_rmse.append(_rmse)
    total_mae = np.array(all_mae).sum()
    total_rmse = np.array(all_rmse).sum()
...
def regressor_detailed_scores_train_one(predictions, gts, raw_df_lst, output_len):
    """
    Desc:
        Some common metrics for regression problems
    Args:
...
def turbine_scores_train(pred, gt, raw_data, examine_len, stride=1):
    """
    Desc:
        Calculate the MAE and RMSE of one turbine
    Args:
