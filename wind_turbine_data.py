...
class WindTurbineData(Dataset):
    ...
    def __clear_data__(self):
        if self.df_raw is not None:
            nan_cond = pd.isna(self.df_raw).any(axis=1)
            zero_cond = self.df_raw['Patv'] < 0
            unknown_1_cond = (self.df_raw['Patv'] == 0) & (self.df_raw['Wspd'] > 2.5)
            unknown_2_cond = (self.df_raw['Pab1'] > 89) & (self.df_raw['Pab2'] > 89) & (self.df_raw['Pab3'] > 89)
            unknown_cond = unknown_1_cond | unknown_2_cond
            abnormal_cond = (self.df_raw['Wdir'] < -180) | (self.df_raw['Wdir'] > 180) | (self.df_raw['Ndir'] < -720) | (self.df_raw['Ndir'] > 720)
            data_cond = nan_cond | zero_cond | unknown_cond | abnormal_cond

            self.df_raw.loc[data_cond, self.df_raw.columns.values[3:]] = 0.0
    ...
    def __get_turbine__(self, turbine_id):
        ...
        ## add by xt
        # ts_map_data_file = './Tmstamp-data.csv'
        # ts_map_data_df = pd.read_csv(ts_map_data_file)
        # ts_map_data = {}
        # for i, j in enumerate(ts_map_data_df['Ts']):
        #     ts_map_data.update({j: float(i + 1)})
        if self.start_col < 3:
            train_data['Tmstamp'] = train_data['Tmstamp'].map(ts_map_data)
        ##
        ...
        ## add by xt
        # if self.start_col < 3:
        #     res_data['Tmstamp'] = res_data['Tmstamp'].map(ts_map_data)
        ##
