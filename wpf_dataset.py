...
Ts_list = [format(i, '02d') +':'+format(j,'02d') for i in range(24) for j in range(0,60,10)]
Ts_to_int = {j: i for i, j in enumerate(Ts_list)}


def filter_valid_data(raw_data):
    cols_name = [n for n in raw_data.columns if 'Day' not in n and 'Tmstamp' not in n and
                 'TurbID' not in n and 'weekday' not in n and 'time' not in n]
    ...
    cond = invalid_cond
    raw_data.loc[cond, cols_name] = 0.0
    return raw_data
...
class PGL4WPFDataset(Dataset):
    ...
    def __read_data__(self):
        ...
        log.info(f"data_shape for x in training: {data_x.shape}")
    ...
    def data_preprocess(self, df_data):
        ...
        # feature_name = [
        #     n for n in df_data.columns
        #     if "Patv" not in n and 'Day' not in n and 'TurbID' not in n
        # ]
        #
        ...
        #
        # new_df_data.loc[:, 'Tmstamp'] = new_df_data.loc[:, 'Tmstamp'].map(Ts_to_int)
        ...
        t = df_data['Tmstamp'].map(Ts_to_int)
    ...
    def build_graph_data(self, df_data):
        ...
        # add filter for valid data
        # df_data = filter_valid_data(df_data)
        ...
        # TODO: data[:, border1s[0]:border2s[0], 2:] is not correct
        # axis=(1,2)
        ...
        # TODO: argpartitin should be argsort
        k = 5
        # topk_indices = np.argpartition(edge_w, -k, axis=1)[:, -k:]

        topk_indices = np.argsort(edge_w, axis=1)[:, -k:]
        ...
        edges = np.concatenate([row.reshape([-1, 1]), col.reshape([-1, 1])],
                               -1)
        # if len(new_row) > 0:
        #     edges = np.concatenate([new_row.reshape([-1, 1]), new_col.reshape([-1, 1])], -1)
        # else:
        #     edges = []
...
class TestPGL4WPFDataset(Dataset):
    ...
    def __read_data__(self):
        ...
        log.info(f"data_shape for x in test: {data_x.shape}")
        self.data_x = data_x
    ...
    def data_preprocess(self, df_data):
        ...
        log.info('adding time')
        # t = df_data['Tmstamp'].apply(func_add_t)
        t = df_data.loc[:, 'Tmstamp'].map(Ts_to_int)
        new_df_data.insert(0, 'time', t)
