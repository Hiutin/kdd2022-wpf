...
class WPFModel(nn.Layer):
    ...
    def __init__(self, config):
        ...
        self.pred_nn_new = nn.Linear(self.capacity, self.capacity)

        # self.graph_input_nn1 = nn.Linear(self.capacity*self.var_len, self.hidden_dims)
        # self.graph_input_nn2 = nn.Linear(self.capacity * self.var_len, self.hidden_dims)

        # self.graph_input_nn1 = nn.Linear(self.hidden_dims, self.hidden_dims)
        # self.graph_input_nn2 = nn.Linear(self.hidden_dims, self.hidden_dims)
    ...
    def forward(self, batch_x, data_mean, data_scale, graph=None):
        ...
        # batch_x = self.graph_input_nn1(batch_x)
        # batch_pred_season = self.graph_input_nn2(batch_pred_season)
        #
        # batch_x = paddle.static.nn.batch_norm(batch_x)
        # batch_pred_season = paddle.static.nn.batch_norm(batch_pred_season)
        ...
        # batch_x_pred = paddle.static.nn.batch_norm(batch_x_pred)
        # batch_x_trends = paddle.static.nn.batch_norm(batch_x_trends)
        #
        #
        # batch_x_pred = self.pred_nn(batch_x_pred)
        # batch_x_trends = self.pred_nn_new(batch_x_trends)
